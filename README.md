...
The `error_name` entity has special behavior.  It is displayed if `show` is set to `true` and when an error occurs, even if `show` is set to `false`. The entity is hidden when its state is:
- `ok`
- `ок`
- `unknown`
- `unavailable`
